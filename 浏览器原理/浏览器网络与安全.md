...
# 二：进程和线程
  ...
  ###  1.一个页面启动的时候最少包含几个进程,4个进程
   - **(1)主进程**：负责管理整个浏览器的UI，标签，书签等基本功能，负责页面显示，用户交互，子进程管理，同时还有存储功能。
   - **(2)GPU进程**：GPU图形处理器，chrome的UI界面是采用GPU来绘制的。
   - **(3)渲染进程**：将HTML,JS,CSS转换为页面，**GUI渲染进程**排版引擎和**JS引擎V8线程**，**时间触发线程**都在该进程中，默认每个tab页都会创建一个渲染进程。
   - **(4)网络进程**：负责页面网络资源加载。
   - 此外还有**插件进程**：负责插件的运行
  ### 2.浏览器的渲染进程中包含哪些线程
...
# 五:浏览器缓存：强缓存diskCache和协商缓存memoryCache
  ### 1.强缓存：
  - 强缓存特点：
  ...
  - 强缓存命中条件：
    - （1）响应头中有**Expires**，Expires是由**http1.0**提出的表示资源过期时间的Header.如果电脑本地时间修改，缓存资源可能会失败！！！
...
# 七：cookie和localStorage的区别
  ...
  | -------------- | ------------------------------------------------------------| -----------------------------| -----------------------------|
  |    存储容量    |                            小,4KB                           |             大,5M            |             大,5M            |
  |    访问权限    |   未设置httpOnly时，可以通过前端js访问，后端读取http请求头  |         仅JS可访问           |         仅JS可访问           |
  |    生命周期    |   无过期时间：关闭浏览器销毁，过期时间expires/max-age       |           永久存储           |    会话关闭就自动删除        |
  |    随请求携带  |               浏览器自动携带同域名下所有cookie              |             手动添加         |             手动添加         |
  |  安全相关属性  |         支持httpOnly/secure/sameSite(防止XSS,CSRF)          |    无安全属性，完全暴露给JS  |    无安全属性，完全暴露给JS  |
...
# 八：浏览器安全
  ...
  - (1)xss攻击案例
    - 恶意注入JS,窃取前端可访问的cookie,token等信息，cookie没有加httpOnly时被黑客通过document.cookie获取。读取localStorage的token等相关数据
    - 监听用户行为，恶意js会使用addEventListener建通键盘事件，获取用户输入的信息
    - 修改DOM，伪造登录页面等
    - 生成浮窗广告
  - (2)恶意脚本注入方式：存储型XSS攻击，反射型XSS攻击，基于DOM的XSS攻击
  ...
  - (1)xss攻击案例
    - **只针对Cookie**,因为浏览器请求自带Cookie,localStorage请求不会自带，必须先通过XSS攻击偷到token
  - (2)阻止攻击：token校验，利用cookie的samesite属性，利用origin,referer属性。
