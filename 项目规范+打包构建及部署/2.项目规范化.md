...
# 四：husky,lint-staged和commitlint结合实现git提交规范
  ### 第一步：huksy安装
    ```js
      npx husky-init // 会自动生成scripts:{"prepare":"husky install"}
    ```
  ### 第二步：安装lint-staged
    ```js
     // lint-staged:是一个在git add到暂存区的文件运行linter的工具，避免在git commit提交时在整个项目执行
      npm i lint-staged -D
    ```
  ### 第三步：在package.json中添加git提交时的lint检测配置,并添加scripts命令
    ```js
      "scripts": {
    ...
    ```
  # 第四步：在husky文件夹中的pre-commit添加命令npm run lint:lint-staged --allow-empty
    ```js
      npm run lint:lint-staged --allow-empty
    ```
  # 第五步：安装commitlint相关插件检查提交消息是否符合commit format
    ```js
      npm i -D @commitlint/cli @commitlint/config-conventional
    ```
  # 第六步： 创建commitlint.config.cjs配置文件,可以参考https://gitee.com/youlaiorg/vue3-element-admin.git
  # 第七步： 执行命令npx husky add .husky/commit-msg 'npx --no-install commitlint --edit "$1"'
    ```js
