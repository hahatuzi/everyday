...
# 三：CommonJS
  ...
  ### 1.CommonJS模块的特点
   ...
   - (4)**每个文件就是一个模块，有自己的作用域。在一个文件里面定义的变量、函数、类，都是私有的，对其他文件不可见。**
  ...
  ### 2.CommonJS语法
    ...
    - (3)加载模块:加载模块使用require方法，**该方法读取一个文件并执行**，返回文件内部module.exports对象.
      ```js
      ...
      ```
      注：不同的实现对require时的路径有不同要求，一般情况可以省略js拓展名，可以使用相对路径，也可以使用绝对路径，甚至可以省略路径直接使用模块名（前提是该模块是系统内置模块）
...
# 四：ESM
  ### 1.为什么js的ESM模块加载像栈？
    JavaScript 的 import 引入机制‌本身并不是栈结构‌，但‌模块加载和执行过程‌中，**‌依赖解析**和**执行顺序**‌确实**遵循‌栈（Stack）的后进先出（LIFO）原则**‌。这主要体现在‌模块依赖图的解析与执行流程‌中。 
  - (1)**模块依赖形成调用栈**‌：当一个模块 A 导入模块 B，而 B 又导入 C 时，加载顺序为 A → B → C。C 首先被加载并执行，然后是 B，最后是 A。这个过程符合“后进先出”的栈特性。
  -  (2)**执行上下文管理**‌：JavaScript 引擎在解析模块依赖时，会将每个待加载模块压入一个隐式的调用栈。只有当前模块完全加载并执行完毕，才会弹出栈顶，继续处理上层模块。
  - (3)**静态依赖分析‌**：ES6 模块（ESM）在编译阶段就确定依赖关系，构建完整的依赖树，并按‌深度优先顺序‌加载，这本质上是栈遍历（DFS）的一种应用 ‌
  ### 2.import()动态导入独立于主模块图，它的加载解析执行全部都是异步的，它引入的模块子图都有自己的依赖顺序
  ### 3.如何在服务端使用import?利用babel插件进行转化
    ```js
